...
def choice(prompt, choices=('y', 'n')):
    while True:
        ...
        prompt = ("\nError found. Please enter a valid option:\n>")
...
def get_filters():
    ...
    while True:
        city = choice("\nFor what city(ies) do you want to select data, "
                      ...
                      "to list multiple.\n>", CITY_DATA.keys())
        month = choice("\nAmong January to June, what month(s) do you "
                       "want do filter data? Use commas to list multiple.\n>",
                       months)
        day = choice("\nFor what week day(s) do you want to filter bikeshare "
                     "data? Use commas to list multiple.\n>", weekdays)
...
def time_stats(df):
    print('\nDisplaying statistics for the most frequent times of '
          'travel...\n')
    start_time = time.time()

    most_common_month = df['Month'].mode()[0]
    print('For the selected filter, the month with the most travel is: ' +
...
def user_stats(df, city):
    ...
    try:
        ...
        print("Oops. There is no data of birth year for {}."
              .format(city.title()))
...
def raw_data(df, mark_place):
    ...
    if mark_place == 0:
        sort_df = choice("\nHow would you like to sort the way the data is "
                         "displayed in the dataframe? Type Enter to view "
                         "unsorted.\n \n [st] Start Time\n [et] End Time\n "
                         "[td] Trip Duration\n [ss] Start Station\n "
                         "[es] End Station\n\n>",
                         ('st', 'et', 'td', 'ss', 'es', ''))
